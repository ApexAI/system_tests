  ...
  size_t number_of_cycles = 100)
{
  ...
  while (rclcpp::ok() && cycle_index < number_of_cycles) {
    ...
    while (rclcpp::ok() && message_index < messages.size()) {
      auto pod_loaned_msg = publisher->borrow_loaned_message();
      auto pod_msg_data = messages[message_index];
      pod_loaned_msg.get() = *pod_msg_data;

      // std::unique_ptr<typename T::_data_type> msg_;
      // msg_ = std::make_unique<typename T::_data_type>();
      // msg_->data = *messages[message_index];

      printf("publishing message #%zu\n", message_index + 1);
      // publisher->publish(*messages[message_index]);
      publisher->publish(std::move(pod_loaned_msg));
      ++message_index;
      message_rate.sleep();
    }
    ++cycle_index;
